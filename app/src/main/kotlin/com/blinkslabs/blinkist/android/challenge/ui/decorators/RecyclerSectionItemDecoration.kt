...
class RecyclerSectionItemDecoration(
    ...
    private val sectionCallback: StickyHeaderSectionCallback
) : ItemDecoration() {
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (!this::headerView.isInitialized) {
            ...
            header = headerView.findViewById(R.id.sectionTitleTextView)
            fixLayoutSize(headerView, parent)
        }
        var previousTitle: CharSequence = ""
        for (i in 0 until parent.childCount) {
            val child = parent.getChildAt(i)
            val position = parent.getChildAdapterPosition(child)
            val title = sectionCallback.getSectionHeader(position)
            header.text = title
            if (previousTitle != title || sectionCallback.isSection(position)) {
